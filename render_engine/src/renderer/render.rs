...
impl Renderer {
    pub fn render(&mut self, encoder: &mut wgpu::CommandEncoder) -> wgpu::SurfaceTexture {
...

        {
            let mut render_pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                ...
                color_attachments: &[Some(wgpu::RenderPassColorAttachment {
                    view: &self.shadow_renderer.texture.view,
                    resolve_target: None,
                    ops: wgpu::Operations {
                ...
                depth_stencil_attachment: Some(wgpu::RenderPassDepthStencilAttachment {
                    view: &self.shadow_renderer.depth_texture,
...

        {
            ...
            render_pass.set_vertex_buffer(0, self.meshes[2].vertex_buffer.slice(..));
            render_pass.set_index_buffer(
                self.meshes[2].index_buffer.slice(..),
                wgpu::IndexFormat::Uint32,
            );
            render_pass.draw_indexed(0..self.meshes[2].index_count, 0, 0..1);
        }
